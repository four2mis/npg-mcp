...
이 서버는 **289개의 MCP 도구**를 27개 카테고리에 걸쳐 노출합니다. 전체 도구 이름, 설명, 입력 매개변수 스키마는 [`tool-schemas.yaml`](tool-schemas.yaml)에 있습니다.

| 카테고리 | 도구 |
|----------|-------|
| **프록시 호스트(Proxy Hosts)** | 41 tools |
| **로그(Logs)** | 32 tools |
...
tool-schemas.yaml  # 289개 도구의 전체 입력 매개변수 스키마
```
