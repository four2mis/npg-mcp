...
async def run():
    ...
    check("get_global_fail2ban: GET endpoint",
          recorded[0][:2] == ("GET", "/api/v1/settings/global-fail2ban")
          and r.get("success") is True,
          recorded[0])

    # partial update: full field set forwarded verbatim
    recorded.clear()
    r = await main.npg_update_global_fail2ban(
        kwargs={"enabled": False, "max_retries": 3, "find_time": 120,
    ...
    check("update_global_fail2ban: single-field body",
          recorded[0][2] == {"action": "notify"}, recorded[0])

    # empty kwargs -> PUT {} (upstream accepts, no-op partial)
    recorded.clear()
    await main.npg_update_global_fail2ban(kwargs={})
    check("update_global_fail2ban: empty body {} allowed",
          recorded[0][2] == {}, recorded[0])

    # unknown field rejected client-side (whitelist enforcement)
    r = await main.npg_update_global_fail2ban(kwargs={"difficulty": 5})
    check("update_global_fail2ban: unknown field rejected",
          r.get("success") is False and "difficulty" in str(r.get("error")), r)

    # strict=False bypasses validation
    recorded.clear()
    await main.npg_update_global_fail2ban(kwargs={"difficulty": 5}, strict=False)
    check("update_global_fail2ban: strict=False bypass",
          recorded[0][2] == {"difficulty": 5}, recorded[0])
    ...
    check("upstream_models frozenset exact",
          ...
          frozenset({"enabled", "max_retries", "find_time",
                     "ban_time", "fail_codes", "action"}),
          TOOL_KWARGS_WHITELIST.get("npg_update_global_fail2ban"))
