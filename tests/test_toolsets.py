...
class TestDerivedDestructiveTools:
    ...
    def test_standard_level_matches_total_minus_destructive(self):
        ...
        assert len(tier_allowed(all_tools, "standard")) == len(all_tools) - len(DESTRUCTIVE_TOOLS) == 237
        # Live surface guardrail (current HEAD): 284 tools, 47 destructive.
        assert len(all_tools) == 284
        assert len(DESTRUCTIVE_TOOLS) == 47
    ...
    def test_standard_and_read_counts_are_stable(self):
        ...
        all_tools = _discover_tool_names()
        assert len(tier_allowed(all_tools, "standard")) == 237
        assert len(tier_allowed(all_tools, "read")) == 131
        assert len(tier_allowed(all_tools, "full")) == 284
