...
| `NPG_TOOL_LEVEL` | `full` | Layered toolset exposure: `read` (133 read-only tools), `standard` (245 tools, no destructive ops), `full` (all 293 tools). Read tools are named `npg_get_*`/`npg_list_*`/`npg_view_*`/`npg_download_*`/`npg_check_*`/`npg_detect_*`. Hidden tools are not listed and not callable. See Toolset Levels below. |
