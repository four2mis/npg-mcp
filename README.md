...
This server exposes **289 MCP tools** across 27 categories. Tool names, descriptions, and full input parameter schemas are in [`tool-schemas.yaml`](tool-schemas.yaml).

| Category | Tools |
|----------|-------|
| **Proxy Hosts** | 41 tools |
| **Logs** | 32 tools |
...
| `NPG_TOOL_LEVEL` | `full` | Layered toolset exposure: `read` (132 read-only tools), `standard` (241 tools, no destructive ops), `full` (all 289 tools). Read tools are named `npg_get_*`/`npg_list_*`/`npg_view_*`/`npg_download_*`/`npg_check_*`/`npg_detect_*`. Hidden tools are not listed and not callable. See Toolset Levels below. |
...
tool-schemas.yaml  # Full input parameter schemas for all 289 tools
```
