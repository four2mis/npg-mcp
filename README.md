...
| `NPG_TOOL_LEVEL` | `full` | Layered toolset exposure: `read` (129 read-only tools), `standard` (230 tools, no destructive ops), `full` (all 279 tools). Read tools are named `npg_get_*`/`npg_list_*`/`npg_view_*`/`npg_download_*`/`npg_check_*`/`npg_detect_*`. Hidden tools are not listed and not callable. See Toolset Levels below. |
