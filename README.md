...
This server exposes **276 MCP tools** across 27 categories. Tool names, descriptions, and full input parameter schemas are in [`tool-schemas.yaml`](tool-schemas.yaml).

| Category | Tools |
|----------|-------|
| **Proxy Hosts** | 36 tools |
| **Logs** | 32 tools |
...
| `NPG_TOOL_LEVEL` | `full` | Layered toolset exposure: `read` (129 read-only tools), `standard` (230 tools, no destructive ops), `full` (all 276 tools). Read tools are named `npg_get_*`/`npg_list_*`/`npg_view_*`/`npg_download_*`/`npg_check_*`/`npg_detect_*`. Hidden tools are not listed and not callable. See Toolset Levels below. |
...
| `standard` | 230 | Everything except destructive operations (all deletes/removes, IP bans, backup restore/upload, password/role/email changes, token revocation, cleanup, reset, session termination, log rotation). Suitable for everyday admin work. |
| `full` | 276 | All tools. Default; behavior without the variable is unchanged. |

Anything else (or unset) falls back to `full`. `tool-schemas.yaml` always documents the full 276-tool reference regardless of the selected level.

## Project Structure

```
npg_mcp/
  main.py       # All 276 MCP tools
...
tool-schemas.yaml  # Full input parameter schemas for all 276 tools
```
